  ...
  return (
    <div className="sheet-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="sheet" role="dialog" aria-modal="true" aria-labelledby="sheet-title">
        ...
        <div className="course-edit">
          <h3>Rules in play</h3>
          <p className="help">Week points {RULES.placePoints.join(' · ')} for 1st–8th, ties on the back 9/6/3 · pairs &amp; scramble add {RULES.pairPoints.join(' · ')} each (ties share) · scramble is net strokes off a team hcp of {RULES.scrambleAllowance[0]}% of the lower CH + {RULES.scrambleAllowance[1]}% of the higher, to one decimal · bonus ball 2× one hole every round (the 18th if not called), +{RULES.bonusKeep} if kept all trip · index ±0.5 per point from {RULES.par} · {RULES.allowance}% allowance. Change these in the code.</p>
        </div>
        <div className="course-edit">
          <div className="btn-row">
